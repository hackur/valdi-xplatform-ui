...
export const ColorUtils = {
   ...
   *
   * Transforms a hex color code to an RGBA string with the specified alpha value.
   *
   ...
   * @example
   * ```typescript
   * const transparent = ColorUtils.hexToRgba('#3B82F6', 0.5);
   * console.log(transparent); // 'rgba(59, 130, 246, 0.5)'
   *
   * const opaque = ColorUtils.hexToRgba('#EF4444', 1);
   * console.log(opaque); // 'rgba(239, 68, 68, 1)'
   * ```
   */
   ...
   *
   * Determines whether light or dark text should be used on a given
   * background color for optimal readability. Uses a simplified
   * luminance-based approach.
   *
   * @param backgroundColor - The background color to check against
   * @returns Either Colors.textInverse (white) or Colors.textPrimary (dark)
   *
   * @example
   * ```typescript
   * const textColor = ColorUtils.getContrastText(Colors.primary);
   * console.log(textColor); // Colors.textInverse (white text on blue)
   *
   * const darkText = ColorUtils.getContrastText(Colors.surface);
   * console.log(darkText); // Colors.textPrimary (dark text on white)
   * ```
   *
   * @remarks
   * For production use with arbitrary colors, consider using a proper
   * color contrast library that implements WCAG contrast ratio calculations.
   */
